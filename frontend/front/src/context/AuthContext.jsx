...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      const response = await fetch('https://pharma-pulse-8vof.onrender.com/api/users/login', {
        method: 'POST',
        headers: {
  ...
  const register = async (name, email, password) => {
    try {
      const response = await fetch('https://pharma-pulse-8vof.onrender.com/api/users/register', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
